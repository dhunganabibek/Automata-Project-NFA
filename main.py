...
def main(): 
    ...
    user_choice = input(f"Enter the name of the file you want to use: \n option A:proj-1-machine.txt (A) \n option B:proj-1-machine-1.txt (B)\n") 
    
    print("-------------------------------------------------------------------------------------------------")
    
    # if user_choice is proj-1-machine.txt or A, it will run NFA with the tuple present in the file
    if(user_choice == "proj-1-machine.txt" or user_choice =='A'): 
        a = "1101"  
        b = "0001"
        c = "1110"
        print("***********NFA************")
        # opening the file proj-1-machine.txt as open_file2 -- contains beta as (1101,0001,1110) 
        with open("proj-1-machine.txt") as open_file2:  
        ...
        state_q0(a) #runs the first string through the NFA
        
        if(rejectedFlag == True):
            print("rejected")
        state_q0(b) #runs the second string through the NFA
        if(rejectedFlag == True):
            print("rejected")
        state_q0(c) #runs the third string through the NFA
        if(rejectedFlag == True):
            print("rejected")

    # if user_choice is proj-1-machine-1.txt or B, it will run NFA with the tuple present in the file and ask for string input.
    if(user_choice == "proj-1-machine-1.txt" or user_choice == "B"):
        print("*******NFA*******")
        # opening file proj-1-machine-1.txt as open_file variable
        with open("proj-1-machine-1.txt") as open_file: 
